...
Uso:
    ros2 launch drone sjtu_drone.launch.py
    ros2 launch drone sjtu_drone.launch.py aruco:=false
    ros2 launch drone sjtu_drone.launch.py spawn_x:=1.0 spawn_z:=0.5
"""
...
def generate_launch_description():
    ...
    sjtu_yaml = os.path.join(pkg_drone, 'config', 'sjtu_drone.yaml')
    aruco_config = os.path.join(pkg_drone, 'config', 'aruco_detector_sjtu_params.yaml')
    ...
    robot_description_config = xacro.process_file(
        xacro_file,
        mappings={'params_path': sjtu_yaml}
    )
    robot_desc = robot_description_config.toxml()

    # Read namespace from yaml
    with open(sjtu_yaml, 'r') as f:
        yaml_dict = yaml.safe_load(f)
    model_ns = yaml_dict.get('namespace', '/drone').lstrip('/')

    # ── Nodes ─────────────────────────────────────────────────────────

    # Robot State Publisher (para TF del dron)
    ...
    static_tf_world_odom = Node(
        ...
        arguments=[spawn_x, spawn_y, '0', '0', '0', '0', 'world', f'{model_ns}/odom'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen',
    )

    # Nodo de control de posición (hover en una posición fija)
    drone_position_controller = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(pkg_drone, 'config', 'drone_position_params.yaml'),
            {'use_sim_time': use_sim_time},
        ],
    )

    # Detector ArUco (reutiliza el existente, topics configurados via params)
    aruco_detector = Node(
        package='drone',
