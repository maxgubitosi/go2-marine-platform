...
similar a como funcionaba la cámara fija anterior.

Topics suscritos:
  /drone/gt_pose (geometry_msgs/Pose) – Ground truth del dron desde Gazebo

Topics publicados:
  /drone/pose (geometry_msgs/PoseStamped) – Pose del dron (compatible con sistema anterior)
  /drone/cmd_vel (geometry_msgs/Twist) – Comandos de posición
  /drone/takeoff (std_msgs/Empty) – Takeoff
  /drone/posctrl (std_msgs/Bool) – Activar position control
"""
...
class DronePositionController(Node):
    def __init__(self):
        ...
        # Suscribirse al estado del dron para confirmar takeoff
        ...
        self.start_time = None  # Se setea cuando recibimos el primer GT pose
    ...
    def gt_pose_callback(self, msg: Pose):
        """Recibe el ground truth pose del sjtu_drone y lo re-publica como PoseStamped."""
        self.current_pose = msg

        # Inicializar start_time con el primer mensaje de GT
        if self.start_time is None:
            self.start_time = self.get_clock().now()
            self.get_logger().info('First GT pose received, starting control sequence...')

        # Re-publicar como PoseStamped para compatibilidad con el pipeline existente
        ps = PoseStamped()
    ...
    def state_callback(self, msg: Int8):
        """Recibe el estado del dron: 0=LANDED, 1=FLYING, 2=TAKINGOFF, 3=LANDING."""
    ...
    def control_loop(self):
        ...
        # Esperar a tener GT pose antes de hacer nada
        if self.start_time is None:
            return

        elapsed = (self.get_clock().now() - self.start_time).nanoseconds / 1e9

        # Paso 1: Esperar un poco y luego hacer takeoff
        # Reenviar takeoff mientras el dron siga en estado LANDED (0)
        ...
        # Paso 2: Activar position control mode después de hover_delay
        ...
        # En modo posctrl, cmd_vel.linear.x/y/z = target position, angular.z = yaw
        cmd = Twist()
